...
export class NameTagObject extends Sprite {
  constructor(text = "", options = {}) {
    const opacity = options.opacity === undefined ? 0.7 : options.opacity; // Default opacity of 0.7
    ...
    this.text = text;
    this.font = options.font === undefined ? "48px Arial" : options.font;
    ...
    if (repaintAfterLoaded && document.fonts && !document.fonts.check(this.font, this.text)) {
      this.paint();
